...
export class Battlefield {
  ...
  private validateSpaceNumber(
    ...
    const maxSpaceNumber =
      this.numPlayersOnTeam === 1 ? ONE_PLAYER_SPACE_MAX : TWO_PLAYER_SPACE_MAX;
  ...
  static fromMongoose(doc: IBattlefield): Battlefield {
    ...
    const spaces = doc.fieldArray.map((space) =>
      BattlefieldSpace.fromMongoose(space)
    );
      ...
      Object.entries(space.connections).forEach(
        ([direction, targetSpaceNumber]) => {
          if (typeof targetSpaceNumber === "number") {
            ...
            if (targetSpace) {
              space.connections[direction as keyof typeof space.connections] =
                targetSpace;
            }
          }
        }
