...
export class BattlefieldService {
    ...
    async createBattlefield(numPlayersOnTeam: 1 | 2): Promise<Battlefield> {
        ...
        const doc = await this.model.create(battlefield.toPrismaObject());
        return Battlefield.fromPrisma(doc);
    }
    ...
    async findBattlefieldById(id: string): Promise<Battlefield> {
        ...
        if (!doc) {
            throw new NotFoundError(
                "Battlefield",
                `Battlefield with id ${id} not found`
            );
        }
        return Battlefield.fromPrisma(doc);
    }

    async updateBattlefieldState(
        id: string,
        updates: Partial<Battlefield>
    ): Promise<Battlefield> {
    ...
    async findBattlefieldsByGameId(gameId: string): Promise<Battlefield[]> {
        ...
        return docs.map((doc) => Battlefield.fromPrisma(doc));
    }
}
